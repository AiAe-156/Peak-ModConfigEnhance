...
internal static class Loc
{
    ...
    internal static readonly Term[] ConfigTerms =
    {
        ...
        new(Plugin.DescTameXUnity, Plugin.DescTameXUnity,
            "默认关闭。打开后每次启动清空 AutoTranslatorConfig.ini 的 Endpoint / FallbackEndpoint，让 XUnity 只做词典翻译（它出厂默认的 GoogleTranslateV2 会机翻全部界面文本），并清空各语言目录下的机翻缓存 _AutoGeneratedTranslations*.txt（原文件备份为 .bak，只留第一次）。只想要词典翻译再打开；依赖在线机翻请保持关闭。",
            "預設關閉。打開後每次啟動清空 AutoTranslatorConfig.ini 的 Endpoint / FallbackEndpoint，讓 XUnity 只做詞典翻譯（它出廠預設的 GoogleTranslateV2 會機翻全部介面文字），並清空各語言目錄下的機翻快取 _AutoGeneratedTranslations*.txt（原檔案備份為 .bak，只留第一次）。只想要詞典翻譯再打開；依賴線上機翻請保持關閉。"),
    };
