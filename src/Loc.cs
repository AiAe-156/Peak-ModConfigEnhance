...
internal static class Loc
{
    ...
    internal static readonly Term[] ConfigTerms =
    {
        ...
        new(Plugin.DescTameXUnity, Plugin.DescTameXUnity,
            "装有 XUnity 时，启动即清空 AutoTranslatorConfig.ini 的 Endpoint / FallbackEndpoint，使其只做词典翻译——出厂默认的 GoogleTranslateV2 会机翻全部界面文本；同时清空各语言目录下的机翻缓存 _AutoGeneratedTranslations*.txt（原文件备份为 .bak）。确实要用机翻请关掉本项。",
            "裝有 XUnity 時，啟動即清空 AutoTranslatorConfig.ini 的 Endpoint / FallbackEndpoint，使其只做詞典翻譯——出廠預設的 GoogleTranslateV2 會機翻全部介面文字；同時清空各語言目錄下的機翻快取 _AutoGeneratedTranslations*.txt（原檔案備份為 .bak）。確實要用機翻請關掉本項。"),
    };
    ...
    internal static LanguageFile Parse(string file)
    {
        ...
        foreach (string raw in File.ReadLines(file, Encoding.UTF8))
        {
            ...
            // 兼容旧版自带/导出词典中的旧按钮名称，只在内存中升级这几个已知文案。
            if (key == "BTN_UNTRANSLATED" && (value == "查漏翻" || value == "补翻译" || value == "補翻譯" || value == "未翻译" || value == "未翻譯"))
            {
                value = result.Code == "zh-tw" || result.Code == "zh-hk" || result.Code == "zh-hant" ? "補漏翻" : "补漏翻";
            }
