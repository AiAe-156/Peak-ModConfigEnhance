...
/// 诊断（1.0.40，确认根因后移除）：1.0.39 已实测「整段同一字体」不能消除末行重叠（排除字体度量）；
/// 现在结构化通告渲染后把逐字符坐标与子网格状态写进日志，用来定位真正重叠的是哪些字。
/// </summary>
internal static class ModConfigDescriptionPanel
{
    ...
    // ── 临时诊断（1.0.40）：定位末行重叠，确认根因后整段移除 ─────────

    private static string? _geometryLoggedFor;

    /// <summary>
    /// 结构化通告渲染后，把每行基线/行高、每个可见字符的坐标与材质序号、残留子网格状态写进日志（同一段文本只记一次）。
    /// 1.0.39 已实测整段同一字体后重叠仍在，根因不是字体度量；这份几何数据用来定位真正重叠的是哪些字。
    /// </summary>
    private static void DumpGeometry(TextMeshProUGUI text)
    {
        if (text.text == _geometryLoggedFor)
        {
            return;
        }

        _geometryLoggedFor = text.text;
        TMP_TextInfo info = text.textInfo;
        Rect rect = text.rectTransform.rect;
        StringBuilder sb = new StringBuilder();
        sb.Append("rectW=").Append(rect.width.ToString("0.#"))
            .Append(" 行数=").Append(info.lineCount)
            .Append(" prefH=").Append(text.preferredHeight.ToString("0.#"))
            .Append(" rendH=").Append(text.renderedHeight.ToString("0.#"));
        for (int i = 0; i < info.lineCount; i++)
        {
            TMP_LineInfo line = info.lineInfo[i];
            sb.Append(" [L").Append(i).Append("]base=").Append(line.baseline.ToString("0.#"))
                .Append(",h=").Append(line.lineHeight.ToString("0.#"))
                .Append(",asc=").Append(line.ascender.ToString("0.#"))
                .Append(",desc=").Append(line.descender.ToString("0.#"));
        }

        sb.Append(" 主网格vtx=").Append(text.mesh != null ? text.mesh.vertexCount : -1);
        foreach (Transform child in text.transform)
        {
            TMP_SubMeshUI sub = child.GetComponent<TMP_SubMeshUI>();
            sb.Append(" 子[").Append(child.name)
                .Append(",on=").Append(child.gameObject.activeInHierarchy)
                .Append(",vtx=").Append(sub != null && sub.mesh != null ? sub.mesh.vertexCount : -1)
                .Append(",f=").Append(sub != null && sub.fontAsset != null ? sub.fontAsset.name : "?")
                .Append(",pos=").Append(((RectTransform)child).anchoredPosition).Append(']');
        }
        ...
        for (int i = 0; i < info.characterCount; i++)
        {
            ...
            sb.Append(c.character).Append("(L").Append(c.lineNumber)
                .Append(",x").Append(c.bottomLeft.x.ToString("0.#")).Append('-').Append(c.topRight.x.ToString("0.#"))
                .Append(",y").Append(c.bottomLeft.y.ToString("0.#")).Append('-').Append(c.topRight.y.ToString("0.#"))
                .Append(",m").Append(c.materialReferenceIndex).Append(") ");
        }

        Plugin.Log.LogWarning($"[面板] 几何 {sb}");
    }
        ...
        private void SetTexts(string title, string description, string meta, bool rich = false)
        {
            ...
            if (Scroll != null)
            {
                if (Description != null)
                {
                    ...
                    if (rich)
                    {
                        DumpGeometry(Description); // 1.0.40 临时诊断：结构化通告的逐字符几何
                    }
                }
