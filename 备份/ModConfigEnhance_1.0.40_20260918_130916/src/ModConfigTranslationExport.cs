...
internal static class ModConfigTranslationExport
{
    ...
    internal static PeakMenuButton MakeToolbarButton(RectTransform page, string name, bool untranslatedOnly, float left, float top, float width, float height)
    {
        ...
        PeakMenuButton button = MakeButton(page, name, untranslatedOnly ? "BTN_UNTRANSLATED" : "BTN_EXPORT", color, left, top, width, height, () => Run(untranslatedOnly));
        button.gameObject.AddComponent<ModConfigSidebarWidgets.TipTarget>().Key = untranslatedOnly ? "TIP_UNTRANSLATED" : "TIP_EXPORT";
        return button;
    }
    ...
    internal static void MakeXUnityButtons(RectTransform page, float left, float top, float width, float height, float gap)
    {
        ...
        toggle.gameObject.AddComponent<ModConfigSidebarWidgets.TipTarget>().Key = "TIP_TOGGLE_ORIG";

        // 重载 = 把磁盘上的语言文件重新读一遍：本模组界面词条重新应用、镜像重写，再让 XUnity 重读它自己的词典。
        // 这样用户改 language\zh-cn.txt 也能立刻见效，不必重新选一次语言（改 XUnity 目录里的文件同样有效）。
        PeakMenuButton reload = MakeButton(page, "MCE_XUnityReload", "BTN_RELOAD", XUnityColor, left + third + gap, top, third, height, () =>
        {
            Loc.ApplyResult result = Loc.Apply(Plugin.LanguageFile.Value, notify: false, refresh: true);
            string message = result.Success ? Loc.Get("NOTICE_RELOADED") + "\n" + result.Message : result.Message;
        ...
        reload.gameObject.AddComponent<ModConfigSidebarWidgets.TipTarget>().Key = "TIP_RELOAD";

        PeakMenuButton openDir = MakeButton(page, "MCE_OpenFolder", "BTN_OPEN_FOLDER", FolderColor, left + (third + gap) * 2f, top, third, height, OpenRootFolder);
        openDir.gameObject.AddComponent<ModConfigSidebarWidgets.TipTarget>().Key = "TIP_OPEN_DIR";
    }
    ...
    internal static PeakMenuButton MakeButton(RectTransform page, string name, string termKey, Color color, float left, float top, float width, float height, Action? onClick)
    {
        ...
        ModConfigSidebarWidgets.FitBorders(button);
    ...
    private static void Run(bool untranslatedOnly)
    {
        try
        {
            ...
            else if (XUnityBridge.CanQuery)
            {
                ...
                // 当前语言组自己的译文也计入（同码变体组各算各的，别组的译文不认）。
                liveQuery = true;
                ...
                isTranslated = text =>
                {
                    ...
                    if (fallback == null)
                    {
                        ...
                        fallback.UnionWith(Loc.CurrentTranslatedTexts());
                    }
            ...
            else
            {
                ...
                set.UnionWith(Loc.CurrentTranslatedTexts());
    ...
    private static List<ModTexts> Collect()
    {
        ...
        foreach (PluginInfo plugin in plugins)
        {
            ...
            {
                mod.Description = ModConfigDescriptionPanel.Panel.ReadModDescription(plugin.Location, showFallback: false);
                result.Add(mod);
            }
        }
    ...
    /// 本模组镜像文件（ModConfigEnhance_*.txt）跳过：它只反映最后激活的一组，同码变体组各算各的。
    /// </summary>
    private static HashSet<string> LoadTranslated(out int fileCount)
    {
        ...
        foreach (string file in Directory.GetFiles(TranslationDirectory, "*.txt", SearchOption.AllDirectories))
        {
            ...
            if (name.StartsWith(Loc.MirrorPrefix, StringComparison.OrdinalIgnoreCase))
            {
                continue; // 本模组镜像只反映最后激活的那组，不能算进别组的覆盖率
            }
            ...
            foreach (string raw in File.ReadLines(file, Encoding.UTF8))
            {
                ...
                string translated = Unescape(line.Substring(eq + 1));
    ...
    private static string Write(List<ModTexts> mods, bool untranslatedOnly, Func<string, bool> isTranslated, bool liveQuery, out int written, out int skipped, out int noNeed)
    {
        ...
        Line("// ==============================================================");
        Line("//");
        Line(L("//   ★ Format: one entry per line — \"original=translation\". Left of = is the original text; write your translation on the right.",
               "//   ★ 基本格式：每行一条「原文=译文」—— = 左边是原文，右边写译文。",
               "//   ★ 基本格式：每行一條「原文=譯文」—— = 左邊是原文，右邊寫譯文。"));
        Line(L("//   ★ Never touch the left side of =: it is the key the dictionary matches character-for-character against on-screen text. Change one character (including writing \\= as =) and the entry can never match — the translation is wasted.",
               "//   ★ 永远不要碰 = 左边：它是词典用来逐字匹配屏幕文字的键，改动任何一个字符（包括把 \\= 写成 =），这条译文就再也对不上原文、等于白翻。",
               "//   ★ 永遠不要碰 = 左邊：它是詞典用來逐字比對螢幕文字的鍵，改動任何一個字元（包括把 \\= 寫成 =），這條譯文就再也對不上原文、等於白翻。"));
        Line(L("//   ★ Inside the original, = and newlines are escaped as \\= and \\n, so a line may contain many \\= (e.g. \"Hold\\=按住显示; Toggle\\=…flag.=\"). The real separator is the first = without a backslash — in a well-formed export, the rightmost one. Use \\= and \\n in your translation too.",
               "//   ★ 原文里的 = 和换行已转义成 \\= 和 \\n，一条里可能有很多个 \\=（如 \"Hold\\=按住显示；Toggle\\=切换…flag.=\"）；真正的分隔符是第一个不带 \\ 的 =，正常导出时就是最右边那个。译文里要用 = 或换行，同样写 \\= 和 \\n。",
               "//   ★ 原文裡的 = 和換行已轉義成 \\= 和 \\n，一條裡可能有很多個 \\=（如 \"Hold\\=按住顯示；Toggle\\=切換…flag.=\"）；真正的分隔符是第一個不帶 \\ 的 =，正常匯出時就是最右邊那個。譯文裡要用 = 或換行，同樣寫 \\= 和 \\n。"));
        Line("//");
        Line(L("//   How to use: see BepInEx\\config\\ModConfigEnhance\\readme.md (translate > save as code[-variant][_part].txt > put in language\\ > Refresh > pick; // display: <name> sets the dropdown label).",
               "//   用法见 BepInEx\\config\\ModConfigEnhance\\readme.md（翻译 → 另存为「语言码[-变体][_分包].txt」→ 放进 language\\ → 刷新 → 选中；首行 // display: 名字 改下拉框显示名）。",
               "//   用法見 BepInEx\\config\\ModConfigEnhance\\readme.md（翻譯 → 另存為「語言碼[-變體][_分包].txt」→ 放進 language\\ → 重新整理 → 選中；首行 // display: 名字 改下拉框顯示名）。"));
        if (untranslatedOnly)
        {
            Line(L("//   Keep existing files. Save additions as a new file such as zh-cn_20_additions.txt, then reload the same language group.",
                   "//   保留已有文件；补翻请另存为 zh-cn_20_补翻.txt，再重载同一语言组。",
                   "//   保留已有檔案；補翻請另存為 zh-tw_20_補翻.txt，再重載同一語言組。"));
        }
        Line("//");
        ...
        if (untranslatedOnly || ModConfigSidebarWidgets.ExportSelectedOnly)
        {
            ...
            {
                Line(liveQuery
                    ? L("//   Untranslated only. MCE_ keys are checked against the active language group; other texts use XUnity's loaded dictionary (exact, regex and substitutions).",
                        "//   只含未翻译条目：MCE_ 键查当前语言组，其余文本查 XUnity 当前词典（精确、正则、替换均计入）。",
                        "//   只含未翻譯條目：MCE_ 鍵查目前語言組，其餘文字查 XUnity 目前詞典（精確、正則、替換均計入）。")
                    : L("//   Untranslated only. MCE_ keys are checked against the active language group; other texts fall back to exact matches in Translation .txt files.",
                        "//   只含未翻译条目：MCE_ 键查当前语言组；其余文本退回按 Translation 下 .txt 原文精确匹配。",
                        "//   只含未翻譯條目：MCE_ 鍵查目前語言組；其餘文字退回按 Translation 下 .txt 原文精確比對。"));
        ...
        HashSet<string>? translatedMceTerms = untranslatedOnly ? Loc.CurrentTranslatedTermKeys() : null;
        written = 0;
        skipped = 0;
        noNeed = 0;
        foreach (Loc.Term term in Loc.Terms)
        {
            if (translatedMceTerms != null && translatedMceTerms.Contains(term.Key))
            {
                skipped++;
                continue;
            }
        ...
        foreach (ModTexts mod in mods)
        {
            ...
            blockCount += Emit(block, mod.Description, seen, isTranslated, untranslatedOnly, ref skipped, ref noNeed);
            foreach ((string Section, List<string> Texts) item in mod.Sections)
            {
