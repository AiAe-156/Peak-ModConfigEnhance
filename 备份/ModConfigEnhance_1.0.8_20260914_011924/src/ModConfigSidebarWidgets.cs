...
internal static class ModConfigSidebarWidgets
{
    ...
    internal sealed class LanguageDropdown : MonoBehaviour
    {
        ...
        internal void Rescan(bool notify)
        {
            ...
            {
                options.Add(_files[i].Display);
                if (string.Equals(_files[i].Stem, current, StringComparison.OrdinalIgnoreCase))
                {
                    selected = i + 1;
                }
            }
        ...
        internal void OnPicked(int index)
        {
            ...
            string selection = index <= 0 || index > _files.Count ? Loc.FollowGame : _files[index - 1].Stem;
            Plugin.LanguageFile.Value = selection;
            Loc.Apply(selection, notify: true, refresh: true);
