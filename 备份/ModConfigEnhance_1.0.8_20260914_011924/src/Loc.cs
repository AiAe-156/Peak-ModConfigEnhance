...
internal static class Loc
{
    ...
    internal static readonly Term[] Terms =
    {
        ...
        new("NOTICE_REFRESHED", "Language folder rescanned: {0} file(s).", "已重新扫描语言目录：{0} 个文件。", "已重新掃描語言目錄：{0} 個檔案。"),
    };
    ...
    /// <summary>目录下的语言文件（不含 _ 开头的模板），按文件名排序。</summary>
    internal static List<LanguageFile> Scan()
    {
        List<LanguageFile> result = new List<LanguageFile>();
        ...
        foreach (string file in files)
        {
            ...
            LanguageFile item = new LanguageFile { Path = file, Stem = Path.GetFileNameWithoutExtension(file) };
            item.Display = ReadDisplayName(file) ?? item.Stem;
            result.Add(item);
        }
    ...
    internal static ApplyResult Apply(string selection, bool notify, bool refresh)
    {
        ...
        try
        {
            ...
            {
                string path = Path.Combine(LanguageDirectory, selection + ".txt");
                if (!File.Exists(path))
                {
                    RegisterBuiltIn();
                    result.Warning = true;
                    result.Message = Format("NOTICE_LANG_ERROR", selection + ".txt", Get("NOTICE_LANG_NOT_FOUND"));
                    return result;
                }
                ...
                {
                    result.Warning = true;
                    result.Message = Format("NOTICE_LANG_ERROR", file.Name, Get("NOTICE_LANG_NO_ENTRIES"));
                    return result;
                }

                result.Message = Format("NOTICE_LANG_APPLIED", file.Display, applied);
                ...
                {
                    result.Warning = true;
                    result.Message += "\n" + Format("NOTICE_LANG_ERROR", file.Name, Format("NOTICE_LANG_BAD_LINES", JoinLines(file.BadLines)));
                }
            }

            result.Mirror = SyncXUnity(selection);
            WriteReadme();
        ...
        {
            Plugin.Log.LogError($"[语言] 应用「{selection}」失败: {ex}");
            RegisterBuiltIn();
            result.Warning = true;
    ...
    private static void ApplyFollowGame()
    {
        RegisterBuiltIn();
        // 三份内建文件用户可能改过：以磁盘为准覆盖对应槽
        OverlayBuiltInFile("en-us", LocalizedText.Language.English);
        OverlayBuiltInFile("zh-cn", LocalizedText.Language.SimplifiedChinese);
        OverlayBuiltInFile("zh-tw", LocalizedText.Language.TraditionalChinese);
    }

    private static void OverlayBuiltInFile(string stem, LocalizedText.Language language)
    {
        ...
        try
        {
            ...
            foreach (Term term in Terms)
            {
                if (file.Entries.TryGetValue(term.Key, out string value))
                {
        ...
        {
            Plugin.Log.LogWarning($"[语言] 读内建文件 {stem}.txt 失败，用内嵌值: {ex.Message}");
        }
    }
    ...
    private static int ApplyFile(LanguageFile file)
    {
        ...
        foreach (Term term in Terms)
        {
            string value = file.Entries.TryGetValue(term.Key, out string v) ? v : term.English;
            if (value != term.English || file.Entries.ContainsKey(term.Key))
            {
                applied++;
            }
    ...
    /// <summary>zh-cn ↔ zh / zh-CN 算一致；zh-tw 遇 zh 一致、遇 zh-CN 不一致。</summary>
    internal static bool SameLanguage(string fileStem, string xunityLanguage)
    {
        string a = fileStem.Trim().ToLowerInvariant().Replace('_', '-');
        string b = xunityLanguage.Trim().ToLowerInvariant().Replace('_', '-');
        if (a == b)
        {
            return true;
        }
