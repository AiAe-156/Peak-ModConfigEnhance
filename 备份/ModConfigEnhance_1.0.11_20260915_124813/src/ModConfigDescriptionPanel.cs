...
internal static class ModConfigDescriptionPanel
{
    ...
    internal sealed class Panel : MonoBehaviour
    {
        ...
        private void ShowOverview(ModdedTABSButton? mod, ModdedTABSButton? section)
        {
            ...
            string summary = Loc.T("Category: ", "当前子分类：", "目前子分類：") + category + "\n\n" + ReadModDescription(location);
            ShowNotice(summary);
            SetTexts(version.Length == 0 ? name : name + "  v" + version, summary, "");
            _overviewShown = true;
            StyleOverviewTitle();
        }
        ...
        private static string ReadModDescription(string pluginLocation)
        {
            try
            {
                if (!string.IsNullOrEmpty(pluginLocation))
                {
                    ...
                    while (directory != null && directory.FullName.StartsWith(root + Path.DirectorySeparatorChar, StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        if (File.Exists(manifest))
                        {
                            ...
                            if (!string.IsNullOrWhiteSpace(info?.description)) return info!.description.Trim();
                            break;
                        }
            ...
            return Loc.T("No description provided in manifest.json.", "未提供 manifest.json 模组说明。", "未提供 manifest.json 模組說明。");
        }

        private void StyleOverviewTitle()
        {
            if (Title != null && _overviewShown)
            {
                Title.color = TimeThemeBridge.IsDark ? new Color(0.88f, 0.75f, 0.47f) : new Color(0.94f, 0.79f, 0.37f);
            }
        }
        ...
        internal void ShowNotice(string text, bool warning = false)
        {
            ...
            if (Title != null) Title.color = MetaColor;
            _shownCell = null;
