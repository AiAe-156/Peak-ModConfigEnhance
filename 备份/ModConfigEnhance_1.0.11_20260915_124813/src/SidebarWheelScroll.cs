...
internal sealed class SidebarWheelScroll : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    internal static bool OnScrollPrefix(ScrollRect __instance, PointerEventData eventData)
    {
        ...
        wheel.Queue(eventData.scrollDelta.y);
        return false;
    }
    ...
    private void Queue(float delta)
    {
        ...
        if (!Mathf.Approximately(current, _lastPosition))
        {
            Cancel();
        }
