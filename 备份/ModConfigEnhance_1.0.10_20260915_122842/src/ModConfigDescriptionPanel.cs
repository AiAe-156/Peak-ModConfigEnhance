...
internal static class ModConfigDescriptionPanel
{
    ...
    internal sealed class Panel : MonoBehaviour
    {
        ...
        private void ShowOverview(ModdedTABSButton? mod, ModdedTABSButton? section)
        {
            ...
            if (Menu?.settings != null)
            {
                foreach (IBepInExProperty item in Menu.settings)
                {
                    ...
                    count++;
                    if (version.Length == 0) version = item.Pluginfo?.Metadata?.Version?.ToString() ?? "";
                }
            }

            string category = section?.category ?? Loc.T("All", "全部", "全部");
            string summary = Loc.T("Category: ", "当前分类：", "目前分類：") + category + "\n" +
                Loc.T("Total settings: ", "配置项总数：", "設定項總數：") + count;
            ShowNotice(summary);
        ...
        private void StyleOverviewTitle()
        {
            ...
            {
                Title.color = TimeThemeBridge.IsDark ? new Color(0.79f, 0.65f, 0.36f) : new Color(0.55f, 0.39f, 0.10f);
            }
        }
