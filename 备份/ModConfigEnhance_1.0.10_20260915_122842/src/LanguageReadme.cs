...
internal static class LanguageReadme
{
    ...
    private const string En = """
        ...
        Selecting a mod or category shows the mod name, version, current category and total setting count above the options. Pointing at or focusing an option restores its own description.
    ...
    private const string ZhCn = """
        ...
        切换模组或分类时，顶部显示模组名、版本、当前分类和配置项总数；移到或选中右侧具体配置项后恢复该选项说明。
    ...
    private const string ZhTw = """
        ...
        切換模組或分類時，頂部顯示模組名、版本、目前分類和設定項總數；移到或選中右側具體設定項後恢復該項說明。
