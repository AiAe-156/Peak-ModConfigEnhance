...
internal static class GamepadSupport
{
    ...
    /// <summary>当前是不是手柄（或非键鼠）方案；InputHandler 不可用时安全返回 false 并记一次日志。</summary>
    internal static bool IsGamepad
    {
        get
        {
            ...
            try
            {
                return InputHandler.GetCurrentUsedInputScheme() != InputScheme.KeyboardMouse;
            }
            catch (Exception ex)
            {
