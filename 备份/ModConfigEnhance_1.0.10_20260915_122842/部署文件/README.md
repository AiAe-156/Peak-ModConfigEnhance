...
> 版本 1.0.10 ｜ 作者 AiAe ｜ 前身是 LocalFix 第 9 分区（LocalFix 2.17.0 起已移除）
...
切换左侧模组或分类时，顶部说明栏显示模组名称、版本、当前分类和配置项总数，名称使用稍突出的颜色。鼠标移到右侧具体配置项或手柄选中配置项后，切回该选项的说明。
...
Selecting a mod or category shows its name, version, current category and total setting count in the description panel, with a subtle name accent. Hovering or focusing a specific setting restores its description.
