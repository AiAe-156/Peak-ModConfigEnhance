...
internal static class ModConfigTreeLayoutPatch
{
    ...
    /// <summary>搜索框底边 -310 下面先放工具行（导出翻译 / 查漏翻 / 仅导出已选；显示原文 / 重载翻译 / 打开目录；语言 ▾ / 刷新），再是列表。关掉翻译导出时没有工具行；「只留语言行」时只剩第三排。</summary>
    private const float ToolbarTop = 318f;

    /// <summary>实际可见的工具行排数：0 无工具行，1 只留语言行，3 全部。</summary>
    private static int VisibleToolbarRows => !Plugin.EnableTranslationExport.Value ? 0
        : Plugin.LanguageRowOnly.Value ? 1 : ModConfigSidebarWidgets.ToolbarRows;

    private static float SidebarTop => VisibleToolbarRows == 0 ? ToolbarTop
    ...
    private static void BuildTree(ModSettingsMenu menu)
    {
        ...
        // 1b. 列表右侧的滚动条
        ModConfigSidebarWidgets.MakeScrollbar(page, modTabs.GetComponent<ScrollRect>(),
        ...
        if (Plugin.EnableTranslationExport.Value)
        {
            try
            {
                sidebar.ExportSelectedBox = ModConfigSidebarWidgets.MakeToolbar(page, SidebarLeft, SidebarWidth, ToolbarTop, Plugin.LanguageRowOnly.Value, _ => sidebar.RefreshColumns());
            }
            catch (Exception ex)
            {
        ...
        if (TimeThemeBridge.Available)
        {
            ...
            list.gameObject.AddComponent<TreeThemeFollower>().Page = page;
