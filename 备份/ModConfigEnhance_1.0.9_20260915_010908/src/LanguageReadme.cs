...
internal static class LanguageReadme
{
    ...
    private const string ZhCn = """
        ...
        补翻流程：保留旧文件 → 查漏翻 → 翻译 → 另存同语言的新后缀文件 → 重载翻译。查漏翻会跳过已翻好的 `MCE_` 界面词条。不要用补翻文件替换原文件；新加一种语言则点「刷新」后选择。
    ...
    private const string ZhTw = """
        ...
        補翻流程：保留舊檔案 → 查漏翻 → 翻譯 → 另存同語言的新後綴檔案 → 重載翻譯。查漏翻會略過已翻好的 `MCE_` 介面詞條。不要用補翻檔案替換原檔案；新增一種語言則點「重新整理」後選擇。
