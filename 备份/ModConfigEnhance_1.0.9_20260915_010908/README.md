...
> 版本 1.0.9 ｜ 作者 AiAe ｜ 前身是 LocalFix 第 9 分区（LocalFix 2.17.0 起已移除）
...
| 翻译工具 | 左栏三排工具行：`导出翻译 / 查漏翻 / 仅导出已选`、`显示原文 / 重载翻译 / 打开目录`、`语言 ▾ / 刷新`；可配置成只留语言行 |
...
- 「查漏翻」不再重复输出已翻好的 `MCE_` 界面词条。保留旧文件 → 查漏翻 → 翻译 → 另存为同语言的新后缀文件 → 重载，不要用补翻文件替换原文件。
