...
> 版本 1.0.16 ｜ 作者 AiAe ｜ 前身是 LocalFix 第 9 分区（LocalFix 2.17.0 起已移除）
...
切换左侧模组或分类时，顶部说明栏显示模组名称、版本、当前子分类和 manifest.json 中的模组说明，概况和配置项说明的标题统一为灰白色，夜间跟随 TimeTheme 配色。鼠标移到右侧具体配置项或手柄选中配置项后，切回该选项的说明。
...
Selecting a mod or category shows its name, version, current subcategory and description from manifest.json in the description panel, using the same gray-white header as setting descriptions, adapting to TimeTheme at night. Hovering or focusing a specific setting restores its description.
