...
internal static class TimeThemeBridge
{
    ...
    internal static void ApplyOptionsTheme(Transform root, bool dark)
    {
        ...
        // 控件与背景 Graphic 不一定在同一节点。分别收集，避免漏掉子节点背景的 ColorBlock。
        HashSet<Graphic> controlTargets = new HashSet<Graphic>();
        foreach (TMP_InputField input in root.GetComponentsInChildren<TMP_InputField>(true))
        {
            PrepareControlTarget(input, controlTargets);
        ...
        {
            PrepareControlTarget(dropdown, controlTargets);
            dropdown.colors = BuildColors(dark, OptionDropdownSelected);
        }

        foreach (Graphic graphic in root.GetComponentsInChildren<Graphic>(true))
        {
            if (graphic == null || controlTargets.Contains(graphic))
            {
                continue;
            }
            ...
            for (int j = 0; j < _optionPairs.Length; j++)
            {
                ...
                if (SameRgb(current, from) || (!dark && SameRgb(current, OptionColor(j, true))))
                {
                    graphic.color = new Color(to.r, to.g, to.b, current.a);
                    break;
                }
    ...
    private static void PrepareControlTarget(Selectable control, HashSet<Graphic> targets)
    {
        Graphic target = control.targetGraphic;
        if (target == null) return;
        targets.Add(target);
        Color current = target.color;
        // 底图保持中性，具体普通/悬停/选中颜色交给 ColorBlock，避免两层紫色相乘或白天残紫。
        foreach ((Color day, Color night) in _optionPairs)
        {
            if (SameRgb(current, day) || SameRgb(current, night) || SameRgb(current, Dim(night)))
            {
                target.color = new Color(1f, 1f, 1f, current.a);
                return;
            }
        }
    }
