...
internal static class ModConfigTranslationExport
{
    ...
    private static string Write(List<ModTexts> mods, bool untranslatedOnly, Func<string, bool> isTranslated, bool liveQuery, out int written, out int skipped, out int noNeed)
    {
        ...
        Line("// ==============================================================");
        Line("//");
        Line(L("//   How to use: see BepInEx\\config\\ModConfigEnhance\\readme.md (translate > save as language-code_note.txt > put in language\\ > Refresh > pick).",
               "//   用法见 BepInEx\\config\\ModConfigEnhance\\readme.md（翻译 → 另存为「语言码_备注.txt」→ 放进 language\\ → 刷新 → 选中）。",
               "//   用法見 BepInEx\\config\\ModConfigEnhance\\readme.md（翻譯 → 另存為「語言碼_備註.txt」→ 放進 language\\ → 重新整理 → 選中）。"));
        if (untranslatedOnly)
        {
