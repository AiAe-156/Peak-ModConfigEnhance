...
- 打字保护的驱动组件（页面开着时每帧看焦点是否在输入框上）此前只在装了 TimeTheme 时才挂载——没装 TimeTheme 的用户在搜索框打字会触发其他模组的快捷键。现在无条件挂载。
- 内建语言文件第二行注释末尾的换行写成了字面量 `\n`，第二、三行挤在一行（只影响观感；已生成的文件只建不覆，可删掉让它重建）。
- The typing-guard driver (marks the search box focused so other mods' hotkeys stay quiet) was only attached when TimeTheme is installed; without TimeTheme, typing in the search box triggered other mods' hotkeys. Now always attached.
- The second comment line of the built-in language files ended with a literal `\n`, gluing lines 2 and 3 together (cosmetic; built-in files are never overwritten — delete one to regenerate it).

### 更改 / Changed

- ModConfig 兼容基线升到 1.8.2（反编译逐行比对：相对 1.8.1 只改了 builderDelegate 里几处版面数值，挂载的方法与字段全部未变；留档 `MODs\反编译\ModConfig_1.8.2_2026-09-18`）。版本不一致时不再刷 Warning，只记一行 Info；真正不兼容的改版仍由「目标方法缺失，跳过」告警。
- 「Disable XUnity machine translation」清空机翻缓存 `_AutoGeneratedTranslations*.txt` 前先留一份 `.bak`（只留第一次的，不覆盖；XUnity 只读 *.txt，不会把它当词典加载），选项说明补记了这一行为。已经是空的文件不再计数。
- manifest.json 依赖改为 Thunderstore 实际包名与当前版本：`BepInEx-BepInExPack_PEAK-5.4.2403`、`PEAKModding-PEAKLib_UI-1.7.1`、`PEAKModding-ModConfig-1.8.2`；新增 256×256 的 `icon.png`（由最终封面缩放）。
- README / 生成的 readme.md 里的演示图链接改为 raw.githubusercontent.com 形式（Thunderstore 不渲染 GitHub blob 页面链接）；依赖说明同步为 ModConfig 1.8.2。
- 清理：删除 ModConfigPaperStyle 里的结构 / 材质诊断日志与早已确认无用的换帧 / Animator 抖动复制路径（抖动来源 2026-09-10 已确认是 `Scouts/UI` 材质本身）；说明面板每帧量单元格四角改为复用数组；刷新钮不再把子对象名写进日志；几处过时的双 `<summary>` 与「分区」措辞修正。
- ModConfig compatibility baseline is now 1.8.2 (decompiled and diffed line by line: only a few layout numbers in builderDelegate changed vs 1.8.1, every hooked method/field is unchanged). A version mismatch now logs one Info line instead of a Warning; genuinely incompatible builds are still reported as "target method missing, skipped".
- "Disable XUnity machine translation" now keeps a one-time `.bak` of each `_AutoGeneratedTranslations*.txt` before emptying it (XUnity only loads *.txt, so the backup is inert); the option description documents this. Already-empty files are no longer counted.
- manifest.json dependencies use the real Thunderstore package names and current versions; added a 256×256 `icon.png`. README demo images switched to raw.githubusercontent.com URLs; dependency notes say ModConfig 1.8.2.
- Cleanup: removed the structure/material diagnostics and the long-dead frame/Animator wobble-copy paths from ModConfigPaperStyle (the wobble is the `Scouts/UI` material itself, confirmed 2026-09-10); the description panel reuses its corner array instead of allocating per frame; the refresh button no longer logs its child names; a few stale doc comments fixed.

## 1.0.41 — 2026-09-18

### 修复 / Fixed

- 说明面板末行重叠（1.0.31 起「导出翻译」悬停注释末行叠字）真正修复。根因不是字体、子网格、行高或遮罩，而是 TMP 默认开启的转义解析（`parseCtrlCharacters`）：注释末行 `…\ModConfigEnhance\readme.md` 里的 `\r` 被当成回车符，TMP 对回车的处理是「x 归零、不换行」，于是 `eadme.md` 从行首回打在 `用法：*\PEA…` 上面；行数、基线、行高、首选高度全部正常，所以此前所有竖向度量检查都"通过"。全量扫描九条注释（三语内建 + 用户语言文件）、全部 cfg 描述与 manifest，含此类序列的只有这一处。现面板三个文本组件一律关闭转义解析（我们喂的换行本来就是真换行），任何含 `\r` `\n` `\t` 的路径都按字面显示。
- 清理排障残留：删除 1.0.40 的几何诊断日志，撤销 1.0.32 为此加的写入后强制重算网格/布局（无必要）。1.0.40 已恢复的原字体（Daruma + 中文回退）与 1.0.39 的「默认」钮昼夜配色保留。
- Description-panel last-line overlap (the "Export texts" tooltip since 1.0.31) is now actually fixed. The cause was never fonts, submeshes, line height or the mask: TMP parses escape sequences by default (`parseCtrlCharacters`), so the `\r` in `…\ModConfigEnhance\readme.md` became a carriage return, which TMP renders as "reset x, stay on the same line" — `eadme.md` was overprinted from the line start onto `Usage: *\PEA…` while line count, baselines, line heights and preferred height all stayed normal. A scan of all nine tooltips (built-in three languages + user language files), every cfg description and every manifest found this to be the only such text. The panel's three text components now disable escape parsing (our newlines are real newlines already), so any path containing `\r` `\n` `\t` renders literally.
- Cleanup: removed the 1.0.40 geometry diagnostics and the 1.0.32 forced mesh/layout rebuild after writes (unnecessary). The original font restored in 1.0.40 and the 1.0.39 Defaults-button day/night colors stay.
